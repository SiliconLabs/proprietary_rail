...
#define WAIT_FOR_RADIO_STATE(state) while (sl_rail_get_radio_state_internal( \
                                             rail_handle) != state) {}
 ...
 */
static void handle_manual_transitions(sl_rail_handle_t rail_handle);
static void handle_auto_transitions(sl_rail_handle_t rail_handle);
static void handle_channel_switch(sl_rail_handle_t rail_handle);
static void handle_rx_complete(sl_rail_handle_t rail_handle);
static void handle_tx(sl_rail_handle_t rail_handle);

/*
 * Set the state transitions for the radio
 */
static void set_state_transitions(sl_rail_handle_t rail_handle,
                                  sl_rail_radio_state_t rx_success,
                                  sl_rail_radio_state_t rx_error,
                                  sl_rail_radio_state_t tx_success,
                                  sl_rail_radio_state_t tx_error);
...
void app_process_action(void)
{
  ...
  if (measurement_requested) {
    ...
    if (rail_status != SL_RAIL_STATUS_NO_ERROR) {
      app_log_error("sl_rail_idle failed with code %lu\n", rail_status);
    }

    if (sl_rail_set_tx_fifo(rail_handle, tx_fifo, TX_FIFO_SIZE, TX_FIFO_SIZE,
...
static void handle_manual_transitions(sl_rail_handle_t rail_handle)
{
  ...
  for (int i = 0; i < REPEAT_CYCLES; i++) {
    rail_status = sl_rail_start_rx(rail_handle, PRIMARY_CHANNEL, NULL);
    if (rail_status != SL_RAIL_STATUS_NO_ERROR) {
      app_log_error("sl_rail_start_rx failed with code %lu\n", rail_status);
    }
    ...
    rail_status = sl_rail_start_tx(rail_handle,
                                   ...
                                   NULL);
    if (rail_status != SL_RAIL_STATUS_NO_ERROR) {
      app_log_error("sl_rail_start_tx failed with code %lu\n", rail_status);
    }
    WAIT_FOR_RADIO_STATE(SL_RAIL_RAC_STATE_TX);

    request_to_TX_active[i] = GET_BUSY_TIME();
    request_to_PA_active[i] = GET_BUSY_OR_READY_TIME();
    WAIT_FOR_RADIO_STATE(SL_RAIL_RAC_STATE_OFF);
...
static void handle_channel_switch(sl_rail_handle_t rail_handle)
{
  ...
  for (int i = 0; i < REPEAT_CYCLES; i++) {
    rail_status = sl_rail_start_rx(rail_handle, SECONDARY_CHANNEL, NULL);
    if (rail_status != SL_RAIL_STATUS_NO_ERROR) {
      app_log_error("sl_rail_start_rx failed with code %lu\n", rail_status);
    }
    WAIT_FOR_RADIO_STATE(SL_RAIL_RAC_STATE_RXSEARCH);
  ...
  WAIT_FOR_RADIO_STATE(SL_RAIL_RAC_STATE_TXPD);
...
static void handle_rx_complete(sl_rail_handle_t rail_handle)
{
  ...
  for (int i = 0; i < REPEAT_CYCLES && !exit_loop; i++) {
    if (sl_rail_set_tx_fifo(rail_handle, tx_fifo, TX_FIFO_SIZE,
                            TX_FIFO_SIZE, 0) != SL_RAIL_STATUS_NO_ERROR) {
      app_log_error("sl_rail_set_tx_fifo failed\n");
    }
    bool error = false;
    while (sl_rail_get_radio_state_internal(rail_handle)
           != SL_RAIL_RAC_STATE_RX2TX) {
      if (measurement_requested) {
        exit_loop = true;
        break;
      }
      if (sl_rail_get_radio_state_internal(rail_handle)
          == SL_RAIL_RAC_STATE_OFF) {
        rail_status = sl_rail_start_rx(rail_handle, PRIMARY_CHANNEL, NULL);
        if (rail_status != SL_RAIL_STATUS_NO_ERROR) {
          app_log_error("sl_rail_start_rx failed with code %lu\n", rail_status);
...
static void handle_tx(sl_rail_handle_t rail_handle)
{
  ...
  uint32_t delay_us = (SL_RAIL_STATE_TRANSITION_TEST_TX_PERIOD
                       * ONE_SECOND_IN_US) / bitrate;

  if (delay_us > SL_RAIL_MAXIMUM_TRANSITION_US) {
    delay_us = SL_RAIL_MAXIMUM_TRANSITION_US - 1;
  } else if (delay_us < SL_RAIL_MINIMUM_TRANSITION_US) {
    delay_us = SL_RAIL_MINIMUM_TRANSITION_US + 1;
  }

  sl_rail_tx_repeat_config_t repeat_config = {
    .iterations = REPEAT_CYCLES * 3,
    .repeat_options = SL_RAIL_TX_REPEAT_OPTION_START_TO_START,
    .delay_or_hop = { .delay_us = delay_us },
  };

  rail_status = sl_rail_set_next_tx_repeat(rail_handle, &repeat_config);
  if (rail_status != SL_RAIL_STATUS_NO_ERROR) {
    app_log_error("sl_rail_set_next_tx_repeat failed with code %lu\n",
                  rail_status);
  }
  ...
  app_log_info(
    ...
    repeat_config.delay_or_hop.delay_us);
                                  ...
                                  sl_rail_radio_state_t tx_error)
{
  ...
  rail_status = sl_rail_set_rx_transitions(rail_handle, &rx_transitions);
  if (rail_status != SL_RAIL_STATUS_NO_ERROR) {
    app_log_error("sl_rail_set_rx_transitions failed with code %lu\n",
                  rail_status);
  }

  rail_status = sl_rail_set_tx_transitions(rail_handle, &tx_transitions);
  if (rail_status != SL_RAIL_STATUS_NO_ERROR) {
    app_log_error("sl_rail_set_tx_transitions failed with code %lu\n",
                  rail_status);
  }
}
