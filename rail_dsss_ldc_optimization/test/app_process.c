...
//                                Global Variables
// -----------------------------------------------------------------------------
/// Flag, indicating transmit request
/// (button has pressed / CLI transmit request has occured)
...
void app_process_action(RAIL_Handle_t rail_handle)
{
  ...
  switch (state) {
    ...
    default:
      // Unexpected state
      app_log_error("Unexpected Simple TRX state occured:%d\n", state);
...
void sl_rail_util_on_event(RAIL_Handle_t rail_handle, RAIL_Events_t events)
{
  ...
  if (events & RAIL_EVENT_CAL_NEEDED) {
    calibration_status = RAIL_Calibrate(rail_handle,
                                        NULL,
...
static void process_rail_errors(void)
{
  ...
  if (rail_cal_error) {
    ...
    app_log_error("Radio Calibration Error occured\nEvents:"
